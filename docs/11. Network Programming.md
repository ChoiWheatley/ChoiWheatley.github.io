...
서버와 연결(`Open_clentfd`)을 맺은 뒤 클라이언트는 루프를 돌면서 stdin으로부터 text line을 입력받고(`Fgets`) 서버에게 전송한다(`Rio_writen`). 서버로부터 응답을 수신하고(`Rio_readlineb`) stdout으로 에코 결과를 출력한다 (`Fputs`)
